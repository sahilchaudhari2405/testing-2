  ...
  async ({ page }) => {
    ...
    try {
      const response = await axiosInstance.get(
        `/users/admin/Customer?page=${next}`
      );
      console.log("order fetched!!", response);
      return response.data;
      // Return the data directly from axios response
...
export const sortOrders = createAsyncThunk(
  "orders/sortOrders",
  async ({ fromDate, toDate, name, selectedView }) => {
    const type = selectedView;
    try {
      const response = await axiosInstance.post("/sales/order/sortOrder", {
        fromDate,
        toDate,
        name,
        type,
...
export const createOrder = createAsyncThunk(
  ...
  async ({
    ...
    try {
      const response = await axiosInstance.post("/sales/order/placeOrder", {
        paymentType,
        BillUser,
        PayId,
        uId,
...
export const createPurchaseOrder = createAsyncThunk(
  ...
  async ({ products, orderDetails }) => {
    // console.log(products)
    // console.log(orderDetails)
    try {
      const response = await axiosInstance.post(
        "/products/product/purchaseOrder",
