...
import {
  handleOfflineCounterSales,
  updateSalesData,
} from "./add.counter.sales.info.js";
import {
  handleAllTotalOfflineSales,
  TotalAllupdateSalesData,
} from "./add.total.sales.info.js";
import {
  handleTotalOfflineSales,
  TotalOfflineupdateSalesData,
} from "./add.offline.sales.info.js";
...
const AddCustomOrder = asyncHandler(async (req, res) => {
  ...
  const {
    ...
    OneUnit,
    payment,
  } = req.body; // Request Body with Custom Info
  try {
    ...
    const OfflineOrder = await getTenantModel(
      tenantId,
      "OfflineOrder",
      offlineOrderSchema
    );
    ...
    if (!order) {
      return res
        .status(404)
        .json(new ApiResponse(404, "Order not found", null));
    }
    ...
    if (orderItem) {
      ...
      let data = {
        ...
        Closing:
          order.paymentType.borrow >= oldItem.finalPriceWithGST
            ? oldItem.finalPriceWithGST
            : 0,
        tenantId: tenantId,
      };
      ...
      (orderItem.type = "custom"), (orderItem.updatedAt = new Date());
    ...
    if (orderItem) {
      ...
      orderItem.totalProfit =
        product.purchaseRate > 1
          ? (OneUnit - product.purchaseRate) * quantity
          : discountedPrice * 0.1;
      orderItem.finalPriceWithGST =
        finalPriceWithGST +
        (OneUnit * quantity * (product.SGST + product.CGST)) / 100;
...
const AddOrder = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    let existingOrderItem = order.orderItems.find(
      (item) => item.product.toString() === product._id.toString()
    );
    ...
    if (existingOrderItem) {
      ...
      existingOrderItem.finalPriceWithGST +=
        oneUnit + (oneUnit * (product.SGST + product.CGST)) / 100;
      existingOrderItem.updatedAt = new Date();

      // Recalculate totalProfit for existing items
      existingOrderItem.totalProfit =
        product.purchaseRate > 1
          ? (oneUnit - product.purchaseRate) * q
          : d * 0.1;
      ...
      const newOrderItem = new OfflineOrderItem({
        ...
        finalPriceWithGST:
          ...
          (product.discountedPrice * (product.SGST + product.CGST)) / 100,
        discountedPrice: product.discountedPrice,
        userId: id,
        totalProfit:
          product.purchaseRate > 1
            ? oneUnit - product.purchaseRate
            : product.discountedPrice * 0.1, // Ensure totalProfit is set
        createdAt: new Date(),
        updatedAt: new Date(),
      });
    ...
    order.paymentType.cash +=
      oneUnit + (oneUnit * (product.SGST + product.CGST)) / 100;
    order.finalPriceWithGST +=
      oneUnit + (oneUnit * (product.SGST + product.CGST)) / 100;
    order.totalPurchaseRate += product.purchaseRate;
    order.totalProfit +=
      product.purchaseRate > 1 ? oneUnit - product.purchaseRate : oneUnit * 0.1;
    order.updatedAt = new Date();
    // Save updated order
    await order.save();
    let Type = "Client";
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, "Product added to order successfully", order));
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json(
        new ApiResponse(500, "Error adding product to order", error.message)
      );
  }
});
...
const placeOrder = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const clientPromise = await createClient(clientReq);
    const cartDeletePromise = Offline_CartItem.deleteMany({
      _id: { $in: cart.cartItems.map((item) => item._id) },
    });
    const cartDelete = Offline_Cart.findByIdAndDelete(cart._id);

    await Promise.all([cartDeletePromise, cartDelete]);
    ...
    const results = await OfflineOrder.findByIdAndUpdate(
        order._id,
        { ClinetID: clientPromise._id }, // Update ClientID
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, "Order placed successfully", results));
  } catch (error) {
    console.error(error);
    return res
      .status(500)
...
const removeItemQuantityOrder = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (cartItem.quantity > 1) {
      ...
      cartItem.finalPriceWithGST -=
        oneUnit + (oneUnit * (product.CGST + product.SGST)) / 100;
      ...
      if (cart) {
        const cartItemExists = cart.orderItems.some(
        ...
        if (cartItemExists) {
          if (cartItem.quantity > 0 && cartItemExists) {
            ...
            cart.finalPriceWithGST -=
              oneUnit + (oneUnit * (product.CGST + product.SGST)) / 100;
            let remainingAmount =
              oneUnit + (oneUnit * (product.CGST + product.SGST)) / 100; // Start with the item's price
...
const RemoveOneItemOnOrder = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    let data = {
      ...
      Closing:
        cart.paymentType.borrow >= cartItem.finalPriceWithGST
          ? cartItem.finalPriceWithGST
          : 0,
      tenantId: tenantId,
    };
    ...
    if (cartItem.quantity > 0 && cartItemExists) {
      ...
      (cart.user = id), cart.orderItems.pull(cartItem._id);
      (cart.orderStatus = "Update"), (cart.totalPrice -= cartItem.price);
      ...
      (cart.totalItem -= 1), (cart.discount -= discount);
      ...
      await cart.save();
      (product.quantity += cartItem.quantity), product.save();
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, "Cart item deleted successfully", cartItem, cart)
      );
  } catch (error) {
    console.error(error);
...
const getOrderById = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json(new ApiResponse(500, "Error retrieving order", error.message));
  }
});
...
const cancelledOrder = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, "Order cancelled successfully", cart));
  } catch (error) {
    console.error(error);
    return res.status(500).json({ success: false, message: error.message });
  }
...
export {
  placeOrder,
  removeItemQuantityOrder,
  RemoveOneItemOnOrder,
  getOrderById,
  updateOrder,
  cancelledOrder,
  AddOrder,
